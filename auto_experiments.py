...
TEST_RUN_EPOCHS = False # Wether to force the number of epochs below. Useful to test for errors without having to wait for hours of training
TEST_RUN_EPOCH_NR = 1
LOAD_DATA = True # Wether to add results to previous .csv data
verbosity = 0
...
for config_string in ground_config_strings:    
    ...
    if ground_config['loss_function'] != 'CCE':
        ...
        #5
        regularizer_list = [None,keras.regularizers.l2(0.01),activity_regularizer_default,keras.regularizers.l1(0.01),keras.regularizers.l1(10**-4)]
    ...
    if ground_config['loss_function'] != 'CCE':
        ...
        gaussian_noise_sigma_strings=["lambda SD: 0.01","lambda SD: 0.02","lambda SD: 0.05","lambda SD: 0.1","lambda SD: 0.2","lambda SD: (0.01/SD)*100","lambda SD: (0.02/SD)*100","lambda SD: (0.05/SD)*100","lambda SD: (0.1/SD)*100","lambda SD: (0.2/SD)*100"]
        gaussian_noise_sigma_list=[lambda SD: 0.01, lambda SD: 0.02, lambda SD: 0.05, lambda SD: 0.1, lambda SD: 0.2,lambda SD: (0.01/SD)*100, lambda SD: (0.02/SD)*100, lambda SD: (0.05/SD)*100, lambda SD: (0.1/SD)*100, lambda SD: (0.2/SD)*100]
...
def train_network(epochs,df,hard_evidence,activation_types,hidden_layers,encoding_dim,sizes_sorted,loss_function,training_method,activity_regularizer,input_layer_type,labeled_data_percentage,VAE,CNN,kernel_landmarks,CNN_layers,CNN_filters,CNN_kernel_size,gaussian_noise_sigma):
    ...
    if training_method=='supervised':
        ...
        x_train, x_train_nolabel, y_train, _ = sklearn.model_selection.train_test_split(df,hard_evidence, test_size= (100-labeled_data_percentage)/100 ) # semi supervised
    else:
...
#         autoencoder.compile(optimizer='adam',loss=loss_function, metrics=['accuracy'],run_eagerly=True) 
    ...
    if training_method=='supervised' or training_method=="supervised_2_percent":
        autoencoder.fit(x_train, y_train, epochs=epochs, batch_size=32, shuffle=True,  callbacks=[hist],verbose=verbosity)
    elif training_method=="semi" or training_method=="semi_supervised" or training_method=="semisupervised":
        # SEMI SUPERVISED
        autoencoder.fit(x_train_nolabel, x_train_nolabel, epochs=epochs, batch_size=32, shuffle=True, callbacks=[hist],verbose=verbosity)
        autoencoder.fit(x_train, y_train, epochs=epochs, batch_size=32, shuffle=True,  callbacks=[hist],verbose=verbosity)
    elif training_method=="unsupervised":
        # UNSUPERVISED
        autoencoder.fit(x_train, x_train, epochs=epochs, batch_size=32, shuffle=True,  callbacks=[hist],verbose=verbosity)
    elif training_method=="semi_sup_first":
        autoencoder.fit(x_train, y_train, epochs=epochs, batch_size=32, shuffle=True,  callbacks=[hist],verbose=verbosity)
        autoencoder.fit(x_train_nolabel, x_train_nolabel, epochs=epochs, batch_size=32, shuffle=True, callbacks=[hist],verbose=verbosity)
    elif training_method=="semi_mixed":
        for i in range(epochs):
            autoencoder.fit(x_train_nolabel, x_train_nolabel, epochs=1, batch_size=32, shuffle=True, callbacks=[hist],verbose=verbosity)
            autoencoder.fit(x_train, y_train, epochs=1, batch_size=32, shuffle=True,  callbacks=[hist],verbose=verbosity)
    else:
        raise Exception("Invalid training method")
    return autoencoder
...
while lowest_results < 10:
    ...
    lowest_results_new = min([len(experiment_config_results[x['mapping']]) for x in experiments if 'no_denoising' in x['full_string']])
...
#     highest_results = max([len(x) for x in experiment_configs_and_results.values()] )
    ...
    for i in (range(len(experiments))):
        ...
        # if previous_runs==lowest_results_new and 'no_denoising' in experiment['full_string']:
            if runs==0:
                print(i)
