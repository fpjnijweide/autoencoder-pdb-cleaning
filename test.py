...
def make_df(use_file, bn, mu, sigma, use_gaussian_noise, use_missing_entry, missing_entry_prob,rows,full_string,sampling_density,gaussian_noise_layer_sigma):
    ...
    if use_file is not None:
        ...
        try:
            ...
            original_database = pd.read_pickle(filename_no_extension+".df")
            hard_evidence = pd.read_pickle(filename_no_extension+".pdb")
            sizes_sorted = list(pd.read_pickle(filename_no_extension+".pkl"))
        except:
            original_database,sizes_sorted,hard_evidence = csv_to_pdb.make_pdb(use_file,sampling_density)
            original_database.to_pickle(filename_no_extension+".df")
            hard_evidence.to_pickle(filename_no_extension+".pdb")
            pd.Series(sizes_sorted).to_pickle(filename_no_extension + ".pkl")
    else:
